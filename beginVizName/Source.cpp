...
int main() {
	...
	// Initialize glad (to load our function pointers)
	if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
		std::cout << "Failed to initialize GLAD" << std::endl;
		return -1;
	}

	// configure global opengl state. With this we can make it so only top most fragments display.
	// -----------------------------
	glEnable(GL_DEPTH_TEST);

	// build and compile our shader program 
	// -------------------------------------------------------------------
	Shader lightingShader("shader.vs.txt", "shader.fs.txt"); // you can name your shader files however you like
	Shader lampShader("1.lamp.vs.txt", "1.lamp.fs.txt");

	// set up vertex data (and buffer(s)) and configure vertex attributes (local space)
	// ------------------------------------------------------------------
	float vertices[] = { //include our simulated perpendicular/normalized vectors
	...
	// first, configure the main cube's VAO (and VBO)
	...
	// second, configure the light's VAO (VBO data stays the same; the vertices are the same for the light object which is also a 3D cube)
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// also draw the lamp object. switches our currently active shader?
		lampShader.use();
